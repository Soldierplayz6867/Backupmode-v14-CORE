        ...
        afkPromises.push(
            guild.setAFKChannel(
                guild.channels.cache.find(
                    (ch: GuildBasedChannel) => ch.name === backupData.afk.name && ch.type === ChannelType.GuildVoice
                ) as VoiceChannel
            )
        );
...
export const loadEmojis = (guild: Guild, backupData: BackupData): Promise<Emoji[]> => {
    ...
    backupData.emojis.forEach((emoji) => {
        if (emoji.url) {
            ...
            emojiPromises.push(
                guild.emojis.create({ name: emoji.name, attachment: Buffer.from(emoji.base64, 'base64') })
            );
        }
    });
        ...
        embedChannelPromises.push(
            guild.setWidgetSettings({
                ...
                channel: <TextChannel | NewsChannel | VoiceChannel | StageChannel | ForumChannel | Snowflake>(
                    guild.channels.cache.find((ch) => ch.name === backupData.widget.channel)
                )
            })
        );
    }
